...
struct ModeSelectionView: View {
    ...
    let onSelect: (ProcyonMode) -> Void

    @Environment(\.dismiss) private var dismiss

    init(
        allowsCancel: Bool = false,
        onSelect: @escaping (ProcyonMode) -> Void
    ) {
        ...
        ScrollView {
            VStack(spacing: 24) {
                ...
                LazyVGrid(
                    ...
                    ForEach(ProcyonMode.allCases) { mode in
        ...
        Button(action: action) {
            ZStack(alignment: .topTrailing) {
                VStack(spacing: 10) {
                    Image(systemName: mode.systemImage)
                        ...
                        .background(.procyonSecondary.opacity(0.14), in: RoundedRectangle(cornerRadius: 16))
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 22, style: .continuous)
                    .stroke(
                        isHovering
                            ? Color.procyonSecondary.opacity(0.9)
