...
func migrateLegacyDefaultsIfNeeded() {
    ...
    for legacySuite in [legacyProcyonSuiteName, legacyCodexForkSuiteName] {
        guard let legacyDefaults = UserDefaults(suiteName: legacySuite) else { continue }
        for (key, value) in legacyDefaults.dictionaryRepresentation() where defaults.object(forKey: key) == nil {
            defaults.set(value, forKey: key)
            migratedKeyCount += 1
        }
    }

    if defaults.string(forKey: ArclumeMode.defaultsKey) == nil,
       let legacyMode = defaults.string(forKey: ArclumeMode.legacyDefaultsKey) {
        defaults.set(legacyMode, forKey: ArclumeMode.defaultsKey)
        migratedKeyCount += 1
    }

    defaults.set(true, forKey: legacyMigrationKey)
    console.log("Migrated \(migratedKeyCount) legacy preference keys to Arclume")
}
