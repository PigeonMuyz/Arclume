...
func getCXBottleConfigFileURL(selectedBottle: String) -> URL? {
    let value = selectedBottle.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !value.isEmpty else { return nil }
    if let url = URL(string: value), url.isFileURL {
        return url.appendingPathComponent("cxbottle.conf")
    }
    return URL(fileURLWithPath: value).appendingPathComponent("cxbottle.conf")
}

func editCXBottleConfigFile(selectedBottle: String, options: [String: String]) throws {
    try upsertCXBottleEnvironment(selectedBottle: selectedBottle, values: options)
}

/// Updates only the requested environment variables. Unlike the old reset path,
/// this preserves all existing CrossOver settings and can insert a missing key.
func upsertCXBottleEnvironment(selectedBottle: String, values: [String: String]) throws {
    guard let configURL = getCXBottleConfigFileURL(selectedBottle: selectedBottle) else {
    ...
    if let index = lines.firstIndex(of: section) {
        sectionStart = index
    } else {
    ...
    let sectionEnd = lines[(sectionStart + 1)...].firstIndex { line in
        line.hasPrefix("[") && line.hasSuffix("]")
    } ?? lines.count
    var matchedKeys = Set<String>()
    for index in (sectionStart + 1)..<sectionEnd {
        for (key, value) in values where lines[index].trimmingCharacters(in: .whitespaces).hasPrefix("\"\(key)\"") {
            lines[index] = toCrossoverENVString(key, value)
            matchedKeys.insert(key)
        }
    }

    let missingLines = values
        .filter { !matchedKeys.contains($0.key) }
        .sorted { $0.key < $1.key }
        .map { toCrossoverENVString($0.key, $0.value) }
    lines.insert(contentsOf: missingLines, at: sectionEnd)
...
func createBottle(cxAppPath: String, bottleName: String = "Steam", template: String = "win10_64") throws -> Process {
    let proc = Process()
    let appURL = URL(fileURLWithPath: cxAppPath)
    let bottleRoot = try getCXPatcherBottlesURL(appDir: appURL)
    try FileManager.default.createDirectory(at: bottleRoot, withIntermediateDirectories: true)
    proc.executableURL = appURL.appendingPathComponent("/Contents/SharedSupport/CrossOver/bin/cxbottle")
    proc.arguments = ["--create", "--bottle", bottleName, "--template", template]
    var environment = ProcessInfo.processInfo.environment
    environment["CX_BOTTLE_PATH"] = bottleRoot.path(percentEncoded: false)
