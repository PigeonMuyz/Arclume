...
private let applicationSupportDirectoryURL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
private let legacyProcyonSupportFolderURL = applicationSupportDirectoryURL.appendingPathComponent("Procyon", isDirectory: true)
private let legacyProcyonLogsFolderURL = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask)
    .first!
...
/// Moves existing Procyon+ data into Arclume without duplicating a Bottle or
...
func migrateLegacyProcyonDataIfNeeded() {
    ...
    migrateLegacyDirectory(
        from: legacyProcyonLogsFolderURL,
...
func migrateLegacyDirectory(from source: URL, to destination: URL) -> Bool {
    ...
    do {
        if !fileManager.fileExists(atPath: destination.path) {
            ...
            console.log("Migrated legacy Procyon data to \(destination.path)")
            return true
        }
        ...
        console.error("Could not migrate legacy Procyon data: \(error.localizedDescription)")
        return false
    }
}
