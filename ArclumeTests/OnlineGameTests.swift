...
struct OnlineGameTests {
    ...
    @Test func folderImportInstallsLauncherInManagedBottleAndCreatesCard() throws {
        ...
        #expect(
            !installedLauncher.standardizedFileURL.path.contains(
                "/drive_c/Program Files/Arclume/JX3Launcher/"
            )
        )
    ...
    @Test func bottleConfigurationSetsChineseLocaleCodepageAndFonts() throws {
        ...
        #expect(!userRegistry.contains("\"1\"=\"00000409\""))
        #expect(!userRegistry.contains("\"1\"=\"00000804\""))
        #expect(userRegistry.contains("[Software\\\\Wine\\\\Mac Driver]"))
        #expect(userRegistry.contains("\"EditMenu\"=\"key\""))
    ...
    @Test func sharedGameLogRetentionRemovesOldestFilesAcrossDirectories() throws {
        ...
        #expect(
            ArclumeGameLogStore.storageUsageBytes(
                in: [currentDirectory, legacyDirectory]
            ) <= 8
        )
    }
